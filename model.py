...
class EdgeUpdateNetwork(nn.Module):
    ...
    def forward(self, node_feat, edge_feat):
        ...
        node = (node - nod).to(tt.arg.device)
        x1 = node.unsqueeze(2)
        ...
        # feat_sim
        high_order = F.softmax(self.feat_sim(node))

        high_order = torch.transpose(high_order, 1, 3)

        mainfold = sim_val * edge_feat[:, 0, :, :] + (1.0 - sim_val) * edge_feat[:, 1, :, :] + high_order * edge_feat[:,
                                                                                                          2, :, :]
        ...
        # set diagonal as zero and normalize
        edge_feat = F.normalize(torch.cat([sim_val, dsim_val, high_order], 1) * edge_feat, p=1, dim=-1) * merge_sum
